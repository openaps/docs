...
* If running 0.7.0 or later, autotune has a --tune-insulin-curve=true option that enables autotune to tune the insulin end time and insulin peak. It tunes the values based on the average deviations observed in the data, the square root of the average of the squared deviations, and the squared average of the square root of the absolute value of the deviations. For each insulin end time hours (DIA) 2 less than the current end time to 2 hours more than the current end time it calculates the deviations values listed. If they all agree in moving the insulin end time in the same direction by 2 or more hours, the insulin end time is moved by 1 hour. A similar process is used to tune insulin peak time. **\*\*SAFETY WARNING\*\*** This tuning method is still very much experimental and not recommended to be run unattended.
