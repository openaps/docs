...
Because running OpenAPS requires frequent communication with your pump, your pump battery tends to drain more quickly than you'd experience when not looping. Some users have had good experiences with Energizer Ultimate Lithium AAA cells (tests show approximately 8.5 days of life) rather than alkaline cells (approximately 2.5 days). Regardless of whether you use alkaline or lithium, you may want to consider a Nightscout alarm to alert you to when the cell is running low. You can do this by setting (in your Nightscout config vars) `PUMP_WARN_BATT_V` and adding `battery` to your `PUMP_FIELDS` so that voltage is displayed on your Nightscout site. For Energizer Ultimate Lithium, a value of 1.31 will give approximatly 10 hours of remaining life until radio communications stop, but the pump will continue working for some time after that. A value of 1.27 will give approximately two hours of remaining life. Note that PUMP_URGENT_BATT_V has a default value of 1.30, so if you do not want to receive urgent alarms, then set this to a very low value like 1.0, or perhaps to 1.27 if you want about two hours of urgent warnings. If you use alkaline cells, then the pump will cease radio communication at 1.22 or 1.21 volts. Setting PUMP_WARN_BATT_V to 1.25 for an alkaline will give about an eight-hour warning until radio communication stops.

If you don't change the cell in time and end up with a "low battery" warning on the pump, the pump will still function, but RF communications will be turned off and you will not be able to loop until you replace the cell.
