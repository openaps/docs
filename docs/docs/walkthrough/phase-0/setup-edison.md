...
### All platforms:
  ...
  - After flashing is complete and the Edison will end the "five minute" process with 100% and say "Download Done" then "rebooting". If that is not the case you can unplug immediately and try again. Be more careful to power the board smoothly, without the lights blinking. Watch the console from the UART port (ssh with PuTTy): you may get asked to type `control-D` to continue after one or more reboots. If so, press `Ctrl-d` to allow it to continue. 
