...
Login as root (with the password you just set above), and run the following lines one by one. The first line "dpkg -P will be quick. Check the printout to see that it ran without error. The apt-get lines will take a long little while):
