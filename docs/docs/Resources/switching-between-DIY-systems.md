 ...
 * **SAFETY WARNING:** If you choose to keep a Loop rig running to bolus via the phone, you MUST turn off Loop's ability to write to write to Nightscout. If you neglect to do this, nightscout will display double entries of carbs and/or boluses and greatly confuse you in the future. Instead, to enter carbs, you can enter directly through Nightscout Care Portal; [use the variety of IFTTT configurations to enter carbs to Nightscout via your Pebble watch, Alexa, Siri, etc.](../walkthrough/phase-4/ifttt-integration.md); or enter using the pump's bolus wizard. 
 * **CONTINUED WARNING**, discovered 2/11/17: You also cannot use Loop app in open loop mode, and enter carbs or bolus from the pump bolus wizard for use in OpenAPS. (See above about turning off Loop's ability to write to Nightscout). You can either turn off Loop's ability to write, or close the app, however reopening the app even a few minutes later may still cause it to double enter to Nightscout. If you do not plan to actively use Loop and DO want to bolus from the pump (via bolus wizard or easy bolus button) with OpenAPS, you should plan to close the Loop app and not run them side-by-side. 
 * **Caution**: You may want to disable the Nightscout COB pill, especially if you are using multiple DIY closed loop systems, as it will likely cause confusion. You should look to the (DIY closed loop system you are using)'s pill for information about COB. This means looking in the OpenAPS or Loop pill for information abou COB. 
