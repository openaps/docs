...
Either:
* After running the ```flashall.bat``` command in the Windows command prompt window, you can hold down the little black button on the underside of the rig next to the battery connector for 10 seconds, which will turn the rig off, and the centre light will go out. Pressing again for 10 seconds will restart the rig and the flash process will continue (the same as rebooting the rig via PuTTY).
